...
export function resolveValueSetForList(
  ...
  return resolvePublisherResource(indexes, { resourceType: 'ValueSet', url });
}
...
export function deriveIndexedListRows(
  ...
  const findValueSet = (url: string): Json | undefined => resolveValueSetForList(url, indexes);
  ...
  for (const vs of localValueSets) {
    for (const importedUrl of importedValueSetUrls(vs)) {
      valueSetImportRefs.set(importedUrl, [...(valueSetImportRefs.get(importedUrl) || []), { type: 'ValueSet', resource: vs }]);
    }
  }
  ...
  const addValueSetRow = (view: number, vs: Json, refs: ListRef[], local: boolean) => {
    ...
    for (const ref of refs) {
      rows.valueSetRefRows.push({
        ...
        web: pageFor(ref.type, ref.resource.id),
      });
    }
  ...
  const addCodeSystemRow = (view: number, system: string, refs: Json[], local: boolean) => {
    ...
    for (const ref of refs) {
      rows.codeSystemRefRows.push({
        ...
        web: localByUrl.has(ref.url) ? pageFor('ValueSet', ref.id) : externalValueSetWeb(ref, findValueSetSource(ref.url)),
      });
    }
